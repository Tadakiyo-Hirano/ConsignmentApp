class UsersController < ApplicationController
  ...
  def end_tasks
    @search_params = consignment_search_params
    @search_none = consignment_search_none
    @user_consignments = @search_none ? @user.consignments.where('done = ?', true).search(@search_params).page(params[:page]).per(10).order(ship_date: :desc).order(product_code: :desc).order(created_at: :desc) : 
                               @user.consignments.where('done = ?', true).search(@search_params).order(ship_date: :desc).order(product_code: :desc).order(created_at: :desc)
  end
