class CustomersController < ApplicationController
  ...
  def update
    if @customer.update_attributes(customer_params)
      ...
      Consignment.where(['customer_id_number == ?', @customer.id]).update_all(customer_code: Customer.find(@customer.id).code ,customer_name: Customer.find(@customer.id).name) if Consignment.where(customer_id_number: @product.id).present?
