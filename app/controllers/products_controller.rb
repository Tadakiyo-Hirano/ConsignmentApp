class ProductsController < ApplicationController
  ...
  def update
    if @product.update_attributes(product_params)
      ...
      Consignment.where(['CAST(product_id_number AS text) == ?', @product.id.to_s]).update_all(product_code: Product.find(@product.id).code, product_name: Product.find(@product.id).name) if Consignment.where(product_id_number: @product.id).present?
