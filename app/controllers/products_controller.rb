  ...
  def update
    if @product.update_attributes(product_params)
      ...
      if Rails.env.production?
        Consignment.where(['product_id_number::text == ?', @product.id.to_s]).update_all(product_code: Product.find(@product.id.to_i).code, product_name: Product.find(@product.id.to_i).name) if Consignment.where(['product_id_number::text == ?', @product.id.to_s])
