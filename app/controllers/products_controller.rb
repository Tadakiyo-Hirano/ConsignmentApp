class ProductsController < ApplicationController
  ...
  def update
    if @product.update_attributes(product_params)
      product_id = CAST(@product.id AS text)
      ...
      Consignment.where(['CAST(product_id_number AS text) == ?', product_id]).update_all(product_code: Product.find(@product.id).code, product_name: Product.find(@product.id).name) if Consignment.where(product_id_number: @product.id).present?
