class ProductsController < ApplicationController
  ...
  def update
    if @product.update_attributes(product_params)
      ...
      Consignment.where(['CAST(product_id_number AS text) == ?', @product.id]).update_all(product_code: Product.find(@product.id).code, product_name: Product.find(@product.id).name) if Consignment.where('CAST(product_id_number AS text)', @product.id).present?
